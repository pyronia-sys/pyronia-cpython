...
def get_app_name(cmd_line):
    app_name = ""

    if cmd_line == None:
        return app_name
    dirs = cmd_line.split('/')
    app_name = dirs[-1].strip()
...
def plot_file(filename, index=0, timestamps=True, children=True, options=None):
    ...
    '''
    mem_line_label = get_app_name(mprofile['cmd_line'])+mem_label_mods[index % len(mem_label_mods)]

    pl.plot(t, mem, mem_line_styles[index % len(mem_line_styles)] + mem_line_colors[index % len(mem_line_colors)],
